...
<title>MediFlow MS – Appointments</title>
<link
...
.add-form .form-row {
	display: grid;
	grid-template-columns: 1fr 1fr 1.2fr 1fr 1fr 1.5fr auto;
	gap: 12px;
	align-items: end;
}
			...
			<main class="main">
				...
				<div class="page-card">
					...
					<div class="page-card-body">
						<form action="appointment" method="post" class="add-form">
							...
							<div class="form-row">
								...
								<div class="form-group">
									<label>Doctor ID</label> <input type="number" name="doctor_id"
										class="form-control" placeholder="Doctor ID" required>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label>Time</label> <input type="time" name="appointment_time"
										class="form-control" required>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label>Reason</label> <input type="text" name="reason"
										class="form-control" placeholder="Reason for visit" required>
								</div>
								<div class="form-group">
									<label>&nbsp;</label>
					...
					<div class="page-card-header">
						<h3>
							<i class="fa-solid fa-calendar-days"
								style="color: var(--blue); margin-right: 8px"></i>Appointment
							Records
						</h3>
						<a href="appointment" class="btn btn-outline btn-sm"> <i
					...
					<div class="page-card-body">
						...
						<div class="search-toolbar">
							<div class="search-box">
								<i class="fa-solid fa-magnifying-glass"></i> <input type="text"
									id="apptSearch" placeholder="Search by reason..."
						...
						<div style="overflow-x: auto;">
							<table class="data-table" id="apptTable">
								...
								<tbody id="apptBody">
									...
									%>
									<tr>
										<td><strong>#<%=a.getAppointment_id()%></strong></td>
										<td><%=a.getPatient_id()%></td>
										<td><%=a.getDoctor_id()%></td>
										<td><i class="fa-regular fa-calendar"
											style="color: var(--muted); font-size: 11px; margin-right: 4px"></i><%=a.getAppointment_date()%></td>
										<td><i class="fa-regular fa-clock"
											style="color: var(--muted); font-size: 11px; margin-right: 4px"></i><%=a.getAppointment_time()%></td>
										<td><span class="badge <%=badgeClass%>"><%=a.getStatus()%></span></td>
										<td><%=a.getReason()%></td>
										<td style="white-space: nowrap;"><a
											href="appointment?action=edit&id=<%=a.getAppointment_id()%>"
											class="btn btn-warning btn-sm"> <i
												class="fa-solid fa-pen"></i> Edit
										</a> <a
											href="appointment?action=delete&id=<%=a.getAppointment_id()%>"
...
function filterAppointments() {
	const query = document.getElementById('apptSearch').value.toLowerCase().trim();
	const rows  = document.querySelectorAll('#apptBody tr');
	let visible = 0;

	rows.forEach(row => {
		const reason = row.cells[6] ? row.cells[6].textContent.toLowerCase() : '';
		if (reason.includes(query)) {
	...
	if (query === '') {
		countEl.textContent = total + ' appointment' + (total !== 1 ? 's' : '') + ' total';
		noResults.style.display = 'none';
	} else {
		countEl.textContent = visible + ' of ' + total + ' appointment' + (total !== 1 ? 's' : '') + ' found';
